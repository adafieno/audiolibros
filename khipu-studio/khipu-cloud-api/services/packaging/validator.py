...
def _check_metadata(file_path: Path) -> Optional[Dict[str, str]]:
    """Extract ID3/metadata tags from audio file."""
    try:
        cmd = [
            'ffprobe',
            '-v', 'error',
        ...
        if not mp3_files:
            ...
            for idx, mp3_file in enumerate(mp3_files, 1):
                ...
                if audio_info:
                    ...
                    # Check ID3 metadata
                    metadata = _check_metadata(mp3_file)
                    if metadata:
                        # Check required tags
                        if not metadata.get('title'):
                            issues.append(ValidationIssue(
