...
// ✅ Centralized cache key generation prevents duplicated logic and inconsistencies
// ✅ Deterministic JSON serialization avoids cache misses due to key ordering
// ✅ Full segment text + voice + processing chain ensures accurate cache matching
// ✅ Same cache key used for preview and playlist generation for consistent caching
// ✅ Cache validation includes all parameters to avoid processing chain bypass issues
//
// CACHE STRATEGY:
...
export class AudioPreviewService {
  ...
  private async previewInternal(options: PreviewOptions, _isExaggerated: boolean = false): Promise<void> {
    try {
      ...
      console.log(`🎤 Voice: ${characterVoice}, Text: "${segmentText.substring(0, 50)}${segmentText.length > 50 ? '...' : ''}"`);
          ...
          if (processedAudioPath) {
            ...
            try {
              ...
              if (!audioData || audioData.byteLength === 0) {
                console.warn('⚠️ Cached processed audio file is empty, will regenerate');
                processedAudioPath = null;
              } else {
                console.log(`✅ Verified cached processed audio: ${audioData.byteLength} bytes - COST SAVINGS!`);
              }
            } catch (verifyError) {
              console.warn('⚠️ Could not verify cached processed audio file, will regenerate:', verifyError);
              processedAudioPath = null;
            }
          }
        } catch {
      ...
      if (!processedAudioPath) {
        ...
        console.log(`🎵 Using cached SoX-processed audio for segment ${options.segmentId}`);
        console.log(`📂 Cache file: ${processedAudioPath}`);
        console.log(`⚡ Skipping processing chain application (using cached result)`);
        console.log(`🎛️ Cached processing matches current chain: ${cacheKey}`);
      }
