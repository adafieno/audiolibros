...
def _string_to_nodes(fragment_xml: str) -> tuple[str, List[ET.Element]]:
    """
    Parse a small XML-ish fragment where text may appear before the first child.
    Returns (leading_text, child_elements). We keep tails on children intact.
    """
    wrap = ET.fromstring(f"<wrap>{fragment_xml}</wrap>")
    return (wrap.text or ""), list(wrap)

def _build_s_nodes(text: str, cad: Dict[str, Any]) -> List[ET.Element]:
    """
    Split into sentences FIRST, then inject <break/> markup inside each sentence.
    Preserve leading text by putting it into s_el.text.
    """
    sentences = _split_to_s_sentences(text)  # <-- split BEFORE injecting tags
    nodes: List[ET.Element] = []
    for s in sentences:
        s_with_breaks = _inject_break_markup(s, cad)  # now safe to inject
        lead, kids = _string_to_nodes(s_with_breaks)

        s_el = ET.Element(f"{{{SSML_NS}}}s")
        if lead:
            s_el.text = lead  # <-- keep "El pueblo dormía..." etc.

        for child in kids:
            s_el.append(child)  # child.tail carries text after each tag
...
) -> ET.Element:  # ← return the final container
    ...
    *,
    ...
    for ln in lines:
        ...
        if ltype in ("sfx", "audio"):
            # SFX: put <audio> under <speak>, but NEVER a top-level <break>
            src = ln.get("src")
            if src:
                ET.SubElement(speak, f"{{{SSML_NS}}}audio", attrib={"src": src})
            if cadence.get("after_block_ms", 0):
                p = ET.SubElement(speak, f"{{{SSML_NS}}}p")
                ET.SubElement(p, f"{{{SSML_NS}}}break", attrib={"time": f'{int(cadence["after_block_ms"])}ms'})
            continue
        ...
        container = _append_voiced_block(  # ← capture the innermost element
            speak,
            voice_id=v_id,
        ...
        if cadence.get("after_block_ms", 0):
            ET.SubElement(container, f"{{{SSML_NS}}}break", attrib={"time": f'{int(cadence["after_block_ms"])}ms'})
