...
Requiere:
  ...
  - llm/client.py con función chat_json(messages=[...], max_tokens=..., strict=...)
...
try:
    from llm.client import chat_json  # tu cliente unificado (OpenAI/otros)
    _HAS_LLM = True
except Exception:
    _HAS_LLM = False
...
def main():
    ...
    ap.add_argument("--no-llm", action="store_true", help="No llamar LLM; solo heurísticas.")
    args = ap.parse_args()
